...
- Request your server administrator or hosting provider to review the origin web server's SSL certificates and verify that:
  - Certificate is not in fact expired.
...
When using [Cloudflare Gateway](/cloudflare-one/traffic-policies/), an HTTP Error `526` might be returned in the [following cases](/cloudflare-one/traffic-policies/troubleshooting/#error-526-invalid-ssl-certificate):
